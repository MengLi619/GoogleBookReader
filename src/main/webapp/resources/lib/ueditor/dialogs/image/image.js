    ...
    UploadImage.prototype = {
        ...
        initUploader: function () {
            ...
            $upload.on('click', function () {
                ...
                if (editor.options.maxImageCount !== undefined && uploadImage.getInsertCount() > editor.options.maxImageCount) {
                    $('.info').html('<span style="color:red;">' + '最多还能上传0个文件'.replace(/[\d]/, editor.options.maxImageCount) + '</span>');
                    return false;
                }
                if (state === 'ready') {
                    uploader.upload();
        ...
        getInsertCount: function () {
            ...
            for (i = 0; file = files[i++]; ) {
                status = file.getStatus();
                if (status == 'queued' || status == 'complete' || status == 'uploading' || status == 'progress') readyFile++;
            }
            return readyFile;
        },
        destroy: function () {
